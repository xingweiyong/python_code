...
def main():
    ...
    for item in source_files:
        destination_dir=re.sub('f1','f2',item) #将源目录差异文件路径替换成备份路径，由于转义字符'\'的问题，源与备份内容不能与文件夹f1、f2重名
        #print destination_dir
        destination_files.append(destination_dir)
        if os.path.isdir(item): #备份目录中创建 不存在的目录,先处理文件夹，之后再用shutil.copy处理文件
    ...
    if createdir_bool:
        ...
        source_files=compareme(dir1,dir2)
        for item in source_files:
            destination_dir=re.sub('f1','f2',item) #将源目录差异文件路径替换成备份路径
            #print destination_dir
            destination_files.append(destination_dir)
    print 'update item:'
    print source_files
