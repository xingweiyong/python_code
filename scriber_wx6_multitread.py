    ...
    def run(self):
        ...
        def OnButtonClick_ok(self):
                ...
                if temp_count < page:
                    ...
                    if re.findall(pattern_str_next,web_get):
                        ...
                        for item in range(0,len(temp_list)):
                            ...
                            get_content('http://club.autohome.com.cn' + temp_list[item])
                            ...
                            wx.CallAfter(self.window.LogMessage, 'page=%s'%temp_count + '    ' + 'url: ' + str(temp_list[item]) + '\n')
                            for i in range(0,num_field):
                                get_regular(list_reg[i],dict_fields[list_field[i]],'http://club.autohome.com.cn' + temp_list[item])
            ...
            for item in list_failed_url1:
                get_content(item)
                wx.CallAfter(self.window.LogMessage,u'失败重下载:   ' + item + '\n')
                time.sleep(random.randint(int(self.window.text_delay.GetValue()),int(self.window.text_delay_end.GetValue())))
                for i in range(0,num_field):
...
class MyFrame(wx.Frame):
    def __init__(self):
        ...
        self.text_monitor = wx.TextCtrl(panel,-1,'begin',pos = (50,355),size = (550,145),style=wx.TE_RICH|wx.TE_MULTILINE)
