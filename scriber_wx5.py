...
class MyFrame(wx.Frame):
    def __init__(self):
        ...
        button_ok1 = wx.Button(panel,-1,'OK',pos = (625,140),size = (45,35))
        button_ok1.Bind(wx.EVT_LEFT_DOWN,self.OnButtonClick_ok1)
    ...
    def get_regular(self,pattern_str,result_list,url):#存放提取内容的列表、正则表达式、当前执行的url
        ...
        if re.findall(pattern,web_get):
            ...
            if the_first_round:
                ...
                result_list.append('')#匹配不上空格补位
                print url
    #OK按钮响应函数，用于获取列表页中正文页并保存为文件
    def OnButtonClick_ok1(self,event):
        ...
        global path2
        path2 = self.text_save.GetValue()[0:self.text_save.GetValue().rfind('\\')]+'/temp_url.txt'
        ...
        for item in temp_list:
            str_url +='http://club.autohome.com.cn/'+ item + '\n'
        print '****'
        print str_url
    ...
    def OnButtonClick_ok(self,event):
        ...
        for item in range(0,len(temp_list)):
            ...
            self.get_content(temp_list[item])
            time.sleep(random.randint(int(self.text_delay.GetValue()),int(self.text_delay_end.GetValue())))
            for i in range(0,num_field):
